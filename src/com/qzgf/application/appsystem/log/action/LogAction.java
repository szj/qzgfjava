...
*/
package com.qzgf.application.appsystem.log.action;
...
import com.qzgf.application.appsystem.log.domain.LogFacade;
...
public class LogAction extends BaseAction{
	...
	public String Grid(){
		...
		List ls=logFacade.findLog(search);
		String data= JSONSerializer.toJSON(ls).toString();
		int count=logFacade.findLogCount(search);
		json="{\"Rows\":"+data+",\"Total\":"+count+"}";
		return viewjson;
	}
	//===============================LG.Ajax专用(为传递参数方便，必须加HashMap search)=================
	//单行记录
	public String GetLog(HashMap search) {
		List ls=logFacade.findLog(search);
	...
	public String Delete(HashMap search){
		AjaxResult ar=null;
		int i=logFacade.deleteLog(search);
		if(i>0){
	...
	public String Edit(){
		AjaxResult ar=null;
		int i=logFacade.updateLog(search);
		if(i>0){
			 ar=AjaxResult.Success("成功");
		}else{
			ar=AjaxResult.Error("失败");
	...
	public String Add(){
		AjaxResult ar=null;
		int i=logFacade.insertLog(search);
		if(i>0){
			 ar=AjaxResult.Success("成功");
		}else{
			ar=AjaxResult.Error("失败");
	...
	/**
	 * Purpose      : 说明
	 * @param logFacade the logFacade to set
	 */
