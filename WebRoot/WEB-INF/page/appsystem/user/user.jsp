...
<body style="padding:10px;height:100%; text-align:center;">
  ...
  <script type="text/javascript">
      ...
      var grid = $("#maingrid").ligerGrid({
          columns: [
          { display: "用户名", name: "username", width: 180, type: "text", align: "left" }, 
          ...
          url: '<%=path%>/appsystem/user!Grid.do', sortName: 'id', 
      ...
      $("#formsearch").ligerForm({
		   fields:[
		   { display: "用户名", name: "username", newline: false, labelWidth: 100, width: 220, space: 30,  cssClass: "field", attr: { "op": "like"} }
		   ],
		   appendID:false,
      ...
      function toolbarBtnItemClick(item) {
          switch (item.id) {
              case "add":
                  top.f_addTab(null, '增加用户信息', '<%=path%>/appsystem/user!Listadd.do');
                  break;
              case "view":
                  var selected = grid.getSelected();
                  if (!selected) { LG.tip('请选择行!'); return }
                  top.f_addTab(null, '查看用户信息', '<%=path%>/appsystem/user!Listdetail.do?search.pid=' + selected.id);
                  break;
              case "modify":
                  var selected = grid.getSelected();
                  if (!selected) { LG.tip('请选择行!'); return }
                  top.f_addTab(null, '修改用户信息', '<%=path%>/appsystem/user!Listedit.do?search.pid=' + selected.id);
                  break;
              case "delete":
