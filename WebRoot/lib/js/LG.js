...
(function ($) {
	...
	LG.ajax = function (options) {
		var p = options || {};
		//var ashxUrl = options.ashxUrl || "/ajax?";//serlvet实现，但传入参数麻烦
		var ashxUrl = options.ashxUrl || "/main!Ajax.do?";
		var url = p.url || ashxUrl + $.param({"search.type":p.type, "search.method":p.method});
		$.ajax({cache:false, async:true, url:url, data:p.data, dataType:"json", type:"post", beforeSend:function () {
			LG.loading = true;
			if (p.beforeSend) {
				p.beforeSend();
			} else {
	...
	LG.createButton = function (options) {
		var p = $.extend({appendTo:$("body")}, options || {});
	...
	LG.bulidFilterGroup = function (form) {
		...
		$(":input", form).not(":submit, :reset, :image,:button, [disabled]").each(function () {
			...
			group.rules.push({op:op, field:name, value:value, type:type});
		});
		return group;
	};
	...
	LG.addSearchButtons = function (form, grid, btn1Container, btn2Container) {
		...
		if (btn1Container) {
			LG.createButton({appendTo:btn1Container, text:"\u641c\u7d22", click:function () {
		...
		if (btn2Container) {
			LG.createButton({appendTo:btn2Container, width:80, text:"\u9ad8\u7ea7\u641c\u7d22", click:function () {
				grid.showFilter();
			}});
		}
	};
	...
	LG.loadForm = function (mainform, options, callback) {
		...
		var p = $.extend({beforeSend:function () {
	...
	LG.submitForm = function (mainform, success, error) {
		...
		if (mainform.valid()) {
			mainform.ajaxSubmit({dataType:"json", success:success, beforeSubmit:function (formData, jqForm, options) {
                    //针对复选框和单选框 处理
	...
	LG.validate = function (form, options) {
		...
		options = $.extend({errorPlacement:function (lable, element) {
			if (!element.attr("id")) {
			...
			$(element).attr("title", lable.html()).ligerTip({distanceX:5, distanceY:-3, auto:true});
	...
	*/
	LG.loadToolbar = function (grid, toolbarBtnItemClick) {
		...
		$.ajax({cache:false, async:true, url:url, 
				data:[{ name: 'search.pid', value: MenuNo }], 
				dataType:"json", type:"post",
		beforeSend:function () {
			LG.loading = true;
			LG.showLoading("正在加载工具条中...");
		}, complete:function () {
			LG.loading = false;
			LG.hideLoading();
		}, success:function (result) {
			...
			if (!result.isError) {
				...
				LG.tip("\u53d1\u73b0\u7cfb\u7edf\u9519\u8bef <BR>\u9519\u8bef\u7801\uff1a" + result.message);
			}
		}});
	};
	...
	LG.overrideGridLoading = function () {
		$.extend($.ligerDefaults.Grid, {onloading:function () {
